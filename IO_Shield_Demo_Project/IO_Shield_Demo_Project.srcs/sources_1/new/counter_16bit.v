...
module counter_16bit(
    ...
    .clk(clk), .clear(clear), .up(up), .dw(dw), .ld(ld), .Din(Din[3:0]), .hex_dec(hex_dec), 
    .q(q[3:0]), .UTC(UTC_t[0]), .DTC(DTC_t[0])
    );
    counter_4bit seven_four (
    .clk(clk), .clear(clear), .up(up & UTC_t[0]), .dw(dw & DTC_t[0]), .ld(ld), .Din(Din[7:4]), 
    .hex_dec(hex_dec), .q(q[7:4]), .UTC(UTC_t[1]), .DTC(DTC_t[1])
    );
